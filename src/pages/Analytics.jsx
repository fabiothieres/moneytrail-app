...
import { BarChart2, PieChart as PieIcon, RefreshCw } from 'lucide-react'
import {
...
export default function Analytics() {
  ...
  const fetchData = useCallback(async () => {
    ...
    // Busca transações do mês atual com categoria — paginado a 100 máx
    const now   = new Date()
    ...
    <Layout>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-surface-muted mt-1">Análise visual do mês atual por categoria</p>
        </div>
        <button
          onClick={fetchData}
          disabled={loading}
      ...
      <p className="text-xs text-surface-muted text-center mt-6">
        Exibindo transações confirmadas do mês atual · máx. 100 registros
      </p>
    </Layout>
  )
}
